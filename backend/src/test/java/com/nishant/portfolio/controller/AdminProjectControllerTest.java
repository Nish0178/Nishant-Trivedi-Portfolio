...
class AdminProjectControllerTest {
    ...
    @Test
    @DisplayName("Repeated GitHub sync execution is idempotent and prevents duplicate projects")
    void testRepeatedGitHubSyncPreventsDuplicates() throws Exception {
        // First sync
        mockMvc.perform(post("/api/admin/projects/sync-github")
                        .header("Authorization", "Bearer " + validToken))
                .andExpect(status().isOk())
        ...
        long countAfterFirstSync = projectRepository.count();
        ...
        mockMvc.perform(post("/api/admin/projects/sync-github")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.addedCount").value(0)); // 0 new projects added!

        long countAfterSecondSync = projectRepository.count();
        assertEquals(countAfterFirstSync, countAfterSecondSync, "Project count must remain identical on repeat sync");
    }

    @Test
    @DisplayName("Creating a project with blank ID generates a clean slug automatically")
    void testCreateProjectWithAutoGeneratedSlug() throws Exception {
        ProjectAdminDto project = new ProjectAdminDto();
        project.setTitle("Autonomous AI Agent System");
        project.setDescription("Self-directed multi-agent orchestration framework.");
        project.setLanguage("Python");
        project.setTechnologies("Python, LangGraph, FastAPI");
        ...
        mockMvc.perform(post("/api/admin/projects")
                        ...
                        .content(objectMapper.writeValueAsString(project)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value("autonomous-ai-agent-system"))
                .andExpect(jsonPath("$.title").value("Autonomous AI Agent System"));

        assertTrue(projectRepository.existsById("autonomous-ai-agent-system"));

        // Cleanup
        projectRepository.deleteById("autonomous-ai-agent-system");
    }
}
